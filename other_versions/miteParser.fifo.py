...
parser.add_argument("--mite_max_len", help="MITE max lenght", type=int, default=650)
parser.add_argument("--tir_align_min_len", help="TIR minimun aligmnent length", type=int, default=10)
...
def _parse(q):
    ...
    while True:
        ...
        record_q = SeqRecord(Seq(seq), id = record_id, description=" a ")
        record_s = SeqRecord(Seq(seq_rc), id = record_id + "_rc", description=" b ")
        ...
        try:
            ...
            #exit()
            print("a")
            print(query_fifo,subject_fifo)
            SeqIO.write(record_q, query_fifo, "fasta")
            #SeqIO.write(record_s, subject_fifo, "fasta")
            print("b")
            # adjust these to reflect the actual work being done in the process substitutions
            # ie. these commands should write the content you want to be read as input by blastn
            #query_proc = subprocess.Popen(redirect_cmd + [query_fifo, 'seq', seq])
            #subject_proc = subprocess.Popen(redirect_cmd + [subject_fifo, 'sec_rc', seq_rc])
